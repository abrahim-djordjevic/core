...
class SettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (state.isLoading && state.currentSettings == null) {
      return Scaffold(
        ...
        body: Center(child: CircularProgressIndicator(color: HudTheme.accentCyan)),
      );
    }
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text('FAILED TO LOAD CONFIGURATION', style: HudTheme.headerCyan.copyWith(color: Colors.redAccent)),
        ...
        children: [
          ...
          if (state.isLoading)
            Container(
              ...
              child: Center(child: CircularProgressIndicator(color: HudTheme.accentCyan)),
            )
        ],
      ),
    );
  ...
  Widget _buildScanSection(ScanSettings scan, SettingsState state, SettingsNotifier notifier, BuildContext context) {
    return _SettingsSection(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Align(alignment: Alignment.centerLeft, child: HudLabel('EXCLUDED PATHS')),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              ...scan.excludedPaths.map((path) => Chip(
              ...
              ActionChip(
                label: Text('+ ADD', style: HudTheme.bodyText.copyWith(color: HudTheme.accentCyan)),
  ...
  Widget _buildAlertsSection(AlertSettings alerts, SettingsState state, SettingsNotifier notifier) {
    return _SettingsSection(
      ...
      child: Column(
        children: [
          _buildSlider('DISK THRESHOLD', alerts.diskThresholdPercent.toDouble(), 1, 100, '%', (val) {
            alerts.diskThresholdPercent = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('RAM THRESHOLD', alerts.ramThresholdPercent.toDouble(), 1, 100, '%', (val) {
            alerts.ramThresholdPercent = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('CPU THRESHOLD', alerts.cpuThresholdPercent.toDouble(), 1, 100, '%', (val) {
            alerts.cpuThresholdPercent = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('THERMAL LIMIT', alerts.thermalThresholdCelsius.toDouble(), 40, 110, '°C', (val) {
            alerts.thermalThresholdCelsius = val.toInt();
            notifier.updateUI();
          }),
          _buildToggle('DESKTOP NOTIFS', alerts.enableDesktopNotifications, (val) {
  ...
  Widget _buildMonitoringSection(MonitoringSettings mon, SettingsState state, SettingsNotifier notifier) {
    return _SettingsSection(
      ...
      child: Column(
        children: [
          _buildSlider('CPU POLL', mon.cpuPollIntervalMs.toDouble(), 500, 60000, ' ms', (val) {
            mon.cpuPollIntervalMs = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('RAM POLL', mon.ramPollIntervalMs.toDouble(), 500, 60000, ' ms', (val) {
            mon.ramPollIntervalMs = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('THERMAL POLL', mon.thermalPollIntervalMs.toDouble(), 500, 60000, ' ms', (val) {
          ...
          if (mon.enableScheduledScans)
            _buildSlider('SCAN INTERVAL', mon.scheduledScanIntervalMinutes.toDouble(), 1, 1440, ' min', (val) {
  ...
  Widget _buildCacheSection(CacheSettings cache, SettingsState state, SettingsNotifier notifier, BuildContext context) {
    return _SettingsSection(
      ...
      child: Column(
        children: [
          _buildSlider('SCAN CACHE TTL', cache.scanCacheTtlMinutes.toDouble(), 1, 1440, ' min', (val) {
            cache.scanCacheTtlMinutes = val.toInt();
            notifier.updateUI();
          }),
          _buildSlider('MAX CACHED SCANS', cache.maxCacheScans.toDouble(), 1, 50, '', (val) {
  ...
  Widget _buildAppearanceSection(AppearanceSettings app, SettingsState state, SettingsNotifier notifier) {
    return _SettingsSection(
      title: 'APPEARANCE',
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Align(alignment: Alignment.centerLeft, child: HudLabel('ACCENT COLOR')),
          const SizedBox(height: 8),
          Row(
            children: [
  ...
  // HELPER WIDGETS
  Widget _buildActionButtons(BuildContext context, SettingsState state, SettingsNotifier notifier) {
    return Column(
      children: [
        SizedBox(
        ...
        SizedBox(
          ...
          child: OutlinedButton(
            ...
            child: const Text('RESET TO DEFAULTS', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1)),
          ),
        ),
      ],
    );
  ...
  void _confirmReset(BuildContext context, SettingsNotifier notifier) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      backgroundColor: HudTheme.bgBase,
      title: Text('INITIATE RESET?', style: HudTheme.headerCyan.copyWith(color: Colors.redAccent)),
      content: Text('This will wipe all configurations and restore factory defaults', style: HudTheme.bodyText),
      actions: [
  ...
  void _showAddPathDialog(BuildContext context, ScanSettings scan, SettingsNotifier notifier) {
    String input = '';
    showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
                ...
                enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: HudTheme.accentCyan)),
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: HudTheme.accentCyan),
              onPressed: () {
  ...
  Widget _colorSwatch(String id, Color color, AppearanceSettings app, SettingsNotifier notifier) {
  ...
  bool _hasError(SettingsState state, String keyword) {
    return state.validationErrors.any((e) => e.toLowerCase().contains(keyword.toLowerCase()));
  }

  Widget _buildErrorMessage(SettingsState state, String keyword) {
    var errors = state.validationErrors.where((e) => e.toLowerCase().contains(keyword.toLowerCase())).toList();
    if (errors.isEmpty) return const SizedBox.shrink();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: errors.map((e) => Padding(
        padding: const EdgeInsets.only(bottom: 8.0),
        child: Text(e, style: HudTheme.bodyText.copyWith(color: Colors.redAccent, fontSize: 12)),
      )).toList(),
    );
  }
}
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (state.validationErrors.any((e) => e.toLowerCase().contains(errorFilter.toLowerCase())))
            ...state.validationErrors.where((e) => e.toLowerCase().contains(errorFilter.toLowerCase())).map((e) => Padding(
              padding: const EdgeInsets.only(bottom: 12), child: Text(e, style: HudTheme.bodyText.copyWith(color: Colors.redAccent, fontSize: 12)),
            )),
          child,
        ],
      ),
